...
struct ExampleAppConsole
{
	...
	bool                  AutoScroll;
	bool                  ScrollToBottom;
	bool				  translate, scale, rotate, name;
	std::string			  nombre;
	lm::vec3			  datos;
	float				  grados;
	script*				  orden;
	...
	ExampleAppConsole()
	{
		...
		Commands.push_back("RANDOM TRANSLATE");
		Commands.push_back("RANDOM ROTATE");
		Commands.push_back("RANDOM SCALE");
		AutoScroll = true;
		ScrollToBottom = true;
	...
	void    ExecCommand( char* command_line)
	{
		...
		{
			ClearLog();
			translate = scale = rotate = name = false;
		}
		...
		else if (Stricmp(command_line, "RANDOM TRANSLATE") == 0)
		{
			AddLog("Please give the entity name\n", command_line);
			translate = true;
			scale = rotate = name = false;
		}
		else if (Stricmp(command_line, "RANDOM ROTATE") == 0)
		{
			AddLog("Please give the entity name\n", command_line);
			rotate = true;
			translate = scale = name = false;
		}
		else if (Stricmp(command_line, "RANDOM SCALE") == 0)
		{
			AddLog("Please give the entity name\n", command_line);
			scale = true;
			translate = rotate = name = false;
		}
		else if (translate && !name) {
			nombre = command_line;
			orden->find_target(nombre);
			orden->randomTranslate();
			translate = scale = rotate = name = false;
		}
		else if (scale && !name) {
			nombre = command_line;
...
public:
	...
	lm::vec3 direction;
	lm::vec3 color;
	float l_att = 0, q_att = 0, p_inner = 0, s_outter = 0;

	//variables materiales
	float spec_glos = 0, normal_fact = 0;
	lm::vec3 ambient;
	lm::vec3 specular;
	lm::vec3 difuse;
	int id_material = -1;
	...
	//variables transform
	float mS = 0;
	float f_Xscale, f_Yscale, f_Zscale = 1;
	float f_Xdegree, f_Ydegree, f_Zdegree = 0;
	float x_degree, y_degree, z_degree = 0;
	float x_scale, y_scale, z_scale = 1;
